...
local function get_subtext_layout(layout, starti, endi, password)
...
function inputbox.draw_text(self)
  ...
  if highlight.start_pos ~= highlight.end_pos then
    ...
    local sub_extent = get_subtext_layout(self._private.layout, highlight.start_pos, highlight.end_pos, self.password_mode)
...
function inputbox:set_text(text)
  ...
  if string.len(text) == 0 then
    txt = self._private.text_hint
  else
    txt = self.password_mode and string.rep(self.password_char, string.len(text)) or text
  end

  local attributes, parsed = Pango.parse_markup('<span foreground="' .. (self._private.fg or beautiful.fg_normal) .. '">' .. txt .. '</span>', -1, 0)
  if not attributes then return parsed.message or tostring(parsed) end

  local len = string.len(txt)
  self._private.layout:set_text(txt, len)
